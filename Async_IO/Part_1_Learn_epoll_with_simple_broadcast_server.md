...
#### Step 6: Time to accept connections

The code we need for this is simple.

- First we need to iterate over the events the poll has populated
...
- Second we need to match and see what is the token for each event.
Right now we expect only 1 event and 1 token, because that is all we have registered.
